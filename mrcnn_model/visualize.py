...
def display_instances1(image, boxes,masks, class_ids, class_names,
                      scores=None, title="",
    ...
    for i in range(N):
        ...
        for verts in contours:
            ...
            #ax.add_patch(p)
            #parr = np.array(parr)
            total_area += math.ceil(area(verts))
        '''ax.text(x1, y1-2, "Area:{}".format(total_area),
...
def display_instances(image, boxes,masks, mask1, class_ids, class_names,
                      scores=None, title="",
                      figsize=(16, 16), ax=None):
    """
    boxes: [num_instance, (y1, x1, y2, x2, class_id)] in image coordinates.
    ...
    for i in range(N):
        ...
        ax.text(x1-7,y1-86+(38*i),"mask_iou for {} :{}".format(label,max(np.array(iou_acc[i]))))
        if label == 'leaf':
...
def display_top_masks(image, mask, class_ids, class_names, limit=6):
...
def plot_overlaps(gt_class_ids, pred_class_ids, pred_scores,
                  overlaps, class_names, threshold=0.9):
    ...
    return [class_names[int(id)] for id in gt_class_ids]
